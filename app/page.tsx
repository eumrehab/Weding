import { CalendarDays, Heart, Home, MapPin, Sparkles, Users } from 'lucide-react';
import './v2.css';
...
const plans = [
  ['신혼집','지역과 입주 시기를 알려주세요','준비 중',Home],
  ['결혼식','날짜와 장소를 알려주세요','예정',MapPin],
  ['웨딩 촬영','촬영 일정을 알려주세요','준비 중',CalendarDays],
];
...
export default function Home(){
  return <main>
    ...
    <section className="hero" id="top">
      ...
      <div className="hero-art"><div className="arch-frame"><div className="photo-placeholder"><Heart size={30} strokeWidth={1.2}/><span>두 사람의 사진</span></div></div><div className="date-card"><span>OUR FIRST GREETING</span><strong>신랑 이름 · 신부 이름</strong></div></div>
    </section>
    ...
    <section className="family-section" id="families">
      ...
      <div className="family-grid"><article><span className="family-number">01</span><Users/><h3>신랑 가족</h3><p>아버지 · 어머니 · 형제자매</p><small>가족 구성과 따뜻한 소개를 알려주세요.</small></article><article><span className="family-number">02</span><Users/><h3>신부 가족</h3><p>아버지 · 어머니 · 형제자매</p><small>가족 구성과 따뜻한 소개를 알려주세요.</small></article></div>
    </section>
    ...
    <section className="life-section" id="plans">
      ...
      <div className="life-grid"><article><Home/><h3>우리의 신혼집</h3><p>생활할 지역과 입주 예정 시기를 정리합니다.</p><strong>내용을 알려주세요</strong></article><article><Heart/><h3>만들고 싶은 가정</h3><p>서로 존중하고, 양가와 따뜻하게 어울리는 가정을 꿈꿉니다.</p><strong>두 분의 생각을 들려주세요</strong></article><article><Users/><h3>가족과 함께하는 시간</h3><p>명절과 가족 행사, 평소 교류에 대한 생각을 담습니다.</p><strong>함께 정리해 보아요</strong></article></div>
    </section>
    ...
    <section className="wedding-section">
      ...
      <div className="plan-grid">{plans.map(([t,d,s,Icon])=><article className="plan-card" key={String(t)}><div className="plan-icon"><Icon/></div><span className="status">{String(s)}</span><h3>{String(t)}</h3><p>{String(d)}</p></article>)}</div>
    ...
    <section className="childhood-section"><header className="section-heading"><p className="eyebrow">LITTLE US</p><h2>어릴 적 우리의 모습</h2><p>닮은 표정과 추억을 찾아보며 편안하게 이야기를 시작해 보세요.</p></header><div className="childhood-grid"><div><Heart/><span>신랑 어린 시절 사진</span></div><div className="childhood-heart">&amp;</div><div><Heart/><span>신부 어린 시절 사진</span></div></div></section>
    <section className="promise-section"><Sparkles/><blockquote>“서로를 존중하고 아끼며,<br/>두 가족과 따뜻하게 어울리는 가정을 만들겠습니다.”</blockquote><p>신랑 이름 · 신부 이름</p></section>
